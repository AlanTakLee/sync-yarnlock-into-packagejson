...
const proccessVersion = (newVersion, currentVersion) => {
  if (options.keepGit && currentVersion.includes('+'))
    return currentVersion;
  if (options.keepLink && currentVersion.includes('link:'))
    return currentVersion;
  if (options.keepVariable && options.keepVariable.split(',').find(f => currentVersion.includes(f)))
    return currentVersion;
...
function updatePackage(jsonPath: string, rootDeps) {
  ...
  const saveTo = path.resolve(path.dirname(jsonPath), options.save ? 'package.json' : 'package.json.yarn');
  ...
  if (!options.save || packageJsonText !== newPackageJsonText) {
